...
export default function PopUpMenu({ menuStyle, rowStyle }) {
  const dispatch = useDispatch();
  const navigation = useNavigation();
  const [rows, setRows] = React.useState([]);
  const [showPopup, setShowPopup] = React.useState(false);

  const translation = new Animated.Value(Dimensions.get("window").height);
  ...
  return (
    <View style={style.Wrapper}>
      <Pressable style={[style.Pressable]} onPress={close}></Pressable>
      <Animated.View
        style={[
          style.Menu,
          menuStyle,
