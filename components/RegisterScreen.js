...
export default function RegisterScreen({ navigation }) {
    ...
    async function PhoneVerification() {
        ...
        // Выполняем запрос
        const json = await APIRequest.post('register/phone/', {
            username: username,
        ...
        if (json.success) {
            ...
            Toast.show({
                ...
                title: _('Error').Title,
                textBody: json.error_text ? t.Error[json.error_text].replace('{seconds}', seconds) : null
            });
        }
    }
                ...
                <DefaultAuthForm content={(
                    <View style={s.Wrapper}>
                        ...
                        <Input wrapperStyle={s.Input}
                            ...
                            onEndEditing={() => setPasswordAgainError(!isPasswordAgainValid)}
                            value={passwordAgain}
                        />
                        <FlatButton text={t.Button}
                                    onPress={() => PhoneVerification()}
                                    wrapperStyle={s.Button}
                                    active={isUsernameValid && isPasswordValid && isPasswordAgainValid && isPhoneNumberValid}/>
                        <View style={s.RegisterOffer}>
                            <Text style={s.RegisterOffer.Text}>
                                {t.AlreadyHaveAnAccount[0]}
                                <Link text={t.AlreadyHaveAnAccount[1]}
                                    onPress={() => {
                ...
                <DefaultAuthForm withoutLogo={true} content={(
                    <View style={s.Wrapper}>
                        ...
                        <Input textInputStyle={s.Input}
                            ...
                            onChangeText={(text) => processForm(null, null, null, null, text)}
